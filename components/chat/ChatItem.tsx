...
const ChatItem = ({ id, content, member, timestamp, fileUrl, deleted, currentMember, isUpdated, socketQuery, socketUrl }: Props) => {
        ...
        <div className="flex flex-col w-full">
          <div className="flex items-center gap-x-2">
            <div className="flex items-center">
              <p onCanPlay={onMemberClick} className="font-semibold text-sm hover:underline cursor-pointer mr-1">
                {member.profile.name}
              </p>
              <ActionTooltip label={member.role}>{ROLE_ICON_MAP[member.role]}</ActionTooltip>
            </div>
            <span className="text-xs text-zinc-50 dark:text-zinc-400">{timestamp}</span>
          </div>
          {isImage && (
            <a
              href={fileUrl}
